...
import { Check, MoreHorizontal, ShieldCheck, UserRound } from "lucide-react";
  ...
  async function updateMember(
    ...
    try {
      ...
      if (action === "promote") {
        setMembers((current) =>
          current.map((item) =>
            item.userId === member.userId ? { ...item, role: "ADMIN" } : item,
          ),
        );
      }
      ...
      if (action === "demote") {
        setMembers((current) =>
          current.map((item) =>
            item.userId === member.userId ? { ...item, role: "MEMBER" } : item,
          ),
        );
      }
    } catch (err) {
                  ...
                  {openMenu === member.id && (
                    <div className="absolute right-0 top-11 z-20 w-52 overflow-hidden rounded-xl border border-border bg-background p-1 shadow-xl">
                      {isAdminMember ? (
                        <button
                          type="button"
                          onClick={() => updateMember(member, "demote")}
                        ...
                        <button
                          type="button"
                          onClick={() => updateMember(member, "promote")}
                          className="w-full rounded-lg px-3 py-2.5 text-left text-sm text-foreground transition-colors hover:bg-muted"
                        >
                          Promote to admin
                        </button>
